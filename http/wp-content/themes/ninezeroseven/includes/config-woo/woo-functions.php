...
if ( !function_exists( 'wbc_template_loop_add_to_cart' ) ) {
	function wbc_template_loop_add_to_cart() {
		...
		if ( in_array( $product->get_type(), array( 'subscription', 'simple' ) ) ) {
			...
			$output .= '<a class="button view-details-btn" href="'.esc_attr( get_permalink( $product->get_id() ) ).'"><i class="fa fa-file-text-o"></i>  '. esc_html__( 'Meer informatie', 'ninezeroseven' ) .'</a>';
		}else {
