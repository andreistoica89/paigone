...
class XLWCTY_Admin_Post_Options {
	...
	public static function setup_fields() {
		...
		$get_fields          = include 'cmb2-wcthankyou-meta-config.php';
		...
		$get_fields = include 'cmb2-settings-config.php';
		...
		$get_fields                 = include 'cmb2-debug-config.php';
		foreach ( $get_fields as $field ) {
			$cmb2_builder_fields_global->add_field( self::settings_add_default_value( $field ) );
		}
	}
	...
	public static function xlwcty_button_html() {
		$new_array = array();
		ob_start();
		$customization_fields = include 'cmb2-wcthankyou-meta-config.php';
                                    ...
                                    <div class="xlwcty_btn_layouts">
                                        <ul>
											...
											foreach ( $is_multiple_arr_count as $key => $val ) {
												?>
                                                <li class='xlwcty_add_more_componets' data-component="<?php echo $key; ?>" data-available-slug='<?php echo wp_json_encode( $val["slug"] ); ?>' data-count="<?php echo 1 ?>" data-max="<?php echo $val["count"] + 1 ?>"><?php _e( $val["title"] ) ?>
            ...
            <div class="xlwcty_template_wrap">
                ...
                <div ng-if="builder_template == 'two_column'">
                    <ul class="xlwcty_layout_prev_content_t xlTemplateUi">
                        <li class="xlwcty_layout_components" ng-repeat="comp in builder_componets.two_column.third" data-slug="{{comp.slug}}" data-component="{{comp.component}}" data-title="{{comp.name}}">
                            {{comp.name}}
                        </li>
                    </ul>
                    ...
                    <div class="xlwcty_layout_prev_content two_column-2col">
                        <ul class="xlwcty_layout_prev_content_l xlTemplateUi">
                            <li class="xlwcty_layout_components" ng-repeat="comp in builder_componets.two_column.first" data-slug="{{comp.slug}}" data-component="{{comp.component}}" data-title="{{comp.name}}">
                                {{comp.name}}
                            </li>
                        </ul>
                        <ul class="xlwcty_layout_prev_content_r xlTemplateUi">
                            <li class="xlwcty_layout_components" ng-repeat="comp in builder_componets.two_column.second" data-slug="{{comp.slug}}" data-component="{{comp.component}}" data-title="{{comp.name}}">
                                {{comp.name}}
                            </li>
                        </ul>
                        <div style="clear:both"></div>
